            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    <thead>
                        ...
                        <th class="text-center col">Nama Toko</th>
                        <th class="text-center col">stok</th>
                        <th class="text-center col">Harga Pangan </th>
                        <th class="text-center col">Total Stok</th>
                    </thead>
                    <tbody class="text-center">
                        <?php $no = 1; ?>
                        @foreach ($data as $datas)
                        <tr>
                            <td class='text-center' rowspan="{{$datas->penjualan->count() + 1}}">{{$datas->nama}}</td>
                            @foreach ($datas->penjualan as $value)
                        <tr>
                            <td class='text-center'>{{$value->toko->nama_toko}}</td>
                            <td class='text-center'>{{$value->jumlah_terjual}}</td>
                            <td class='text-center'>@currency($value->harga)</td>
                            <td class='text-center'>{{$value->jumlah_terjual}}</td>
                        </tr>
                        @endforeach
                        </tr>
